// ---------------------------------------------------------------
// VERSÃO STANDALONE — abre direto no navegador, sem servidor Flask.
// Todas as operações (categorias, usuários, demandas) falam
// diretamente com o Supabase via REST.
// ---------------------------------------------------------------
...
formDemanda.addEventListener("submit", async (evento) => {
  ...
  try {
    const resposta = await fetch(`${SUPABASE_URL}/demandas`, {
      method: "POST",
    ...
    if (resposta.ok) {
      ...
      formErro.textContent = erro.message || "Não foi possível registrar a demanda.";
    }
  } catch (erro) {
...
async function carregarDemandas() {
  ...
  try {
    const resposta = await fetch(`${SUPABASE_URL}/demandas?order=id.desc`, { headers: supabaseHeaders });
